    ...
    @if($ultimoRecorrido)
        <div class="bg-white border border-gray-100 rounded-xl p-4 mb-4">
            <div class="flex items-center justify-between mb-3">
                ...
                <span class="text-2xs text-gray-400">{{ $ultimoRecorrido->fecha_recorrido->format('d/m/Y') }}</span>
            </div>
            <div class="flex flex-wrap gap-1.5 mb-2">
                @foreach($ultimoRecorrido->tecnicos as $t)
                    <span class="text-2xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-600">{{ $t->name }}</span>
                @endforeach
            </div>
            @if($ultimoRecorrido->observacion_general)
                <p class="text-xs text-gray-500 italic">{{ $ultimoRecorrido->observacion_general }}</p>
            @endif
            <a href="{{ route('campo.recorridos') }}" class="inline-block mt-3 text-xs font-medium text-agro-600 hover:text-agro-700">
