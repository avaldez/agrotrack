    ...
    <form wire:submit.prevent="guardar" class="space-y-4">
        ...
        <div class="bg-white border border-gray-100 rounded-xl p-4">
            <div class="mb-3">
                ...
                @error('fechaRecorrido') <span class="text-xs text-red-500 mt-1 block">{{ $message }}</span> @enderror
            </div>
            <div>
                ...
                <div class="flex flex-wrap gap-1.5">
                    @foreach($tecnicosDisponibles as $t)
                        <label class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs cursor-pointer transition
                            {{ in_array($t->id, $tecnicosSeleccionados) ? 'bg-agro-50 border-agro-500 text-agro-700' : 'bg-white border-gray-200 text-gray-500' }}">
                ...
                @error('tecnicosSeleccionados') <span class="text-xs text-red-500 mt-1 block">{{ $message }}</span> @enderror
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-100 rounded-xl p-4">
            <div class="grid grid-cols-1 gap-3">
                <div>
                    ...
                    @error('clienteId') <span class="text-xs text-red-500 mt-1 block">{{ $message }}</span> @enderror
                </div>
                <div>
                    ...
                    @error('parcelaId') <span class="text-xs text-red-500 mt-1 block">{{ $message }}</span> @enderror
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-100 rounded-xl p-4">
            ...
            <div class="grid grid-cols-2 gap-2">
                <button type="button" wire:click="$set('tipoVisita', 'MONITOREO')"
                    ...
                    {{ $tipoVisita === 'MONITOREO' ? 'bg-agro-50 border-agro-500 text-agro-700' : 'bg-white border-gray-200 text-gray-500' }}">
                    <div class="text-lg mb-0.5">🔍</div>
                    Solo monitoreo
                </button>
                <button type="button" wire:click="$set('tipoVisita', 'RECOMENDACION')"
                    class="py-3 px-4 rounded-xl border text-sm font-medium transition text-center
                    {{ $tipoVisita === 'RECOMENDACION' ? 'bg-agro-50 border-agro-500 text-agro-700' : 'bg-white border-gray-200 text-gray-500' }}">
                    <div class="text-lg mb-0.5">📋</div>
                    Con recomendación
                </button>
            </div>
        ...
        <div class="bg-white border border-gray-100 rounded-xl p-4">
            ...
            <div class="space-y-3">
                @foreach([
                    ...
                    <div>
                        <span class="text-xs text-gray-500 mb-1.5 block">{{ $label }}</span>
                        <div class="flex gap-1.5">
                            @foreach(['ok' => '✅ Sin novedad', 'warn' => '⚠️ Leve', 'bad' => '❌ Alta'] as $val => $txt)
                                <button type="button" wire:click="$set('{{ $campo }}', '{{ $val }}')"
                                    ...
                                    {{ ${$campo} === $val ? ($val === 'ok' ? 'bg-agro-50 border-agro-500 text-agro-700' : ($val === 'warn' ? 'bg-amber-50 border-amber-400 text-amber-700' : 'bg-red-50 border-red-400 text-red-700')) : 'bg-white border-gray-200 text-gray-400' }}">
                                    {{ $txt }}
                                </button>
                            @endforeach
                        </div>
        ...
        @if($tipoVisita === 'RECOMENDACION')
            <div class="bg-white border border-gray-100 rounded-xl p-4">
                ...
                @error('productos') <span class="text-xs text-red-500 mb-2 block">{{ $message }}</span> @enderror
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if($prod['producto_id'])
                                    <div class="flex items-center gap-2 mt-1.5">
                                        <input type="number" step="0.01" wire:model.live="productos.{{ $index }}.dosis"
                                            ...
                                            placeholder="Dosis">
                                        <span class="text-2xs text-gray-400">{{ $prod['unidad'] }}/ha</span>
                ...
                @if(auth()->user()->puedeMutar() && $totalCostoHa > 0)
                    <div class="mt-3 p-2.5 bg-amber-50 border border-amber-200 rounded-lg flex items-center justify-between">
                        ...
                        <span class="text-sm font-bold text-amber-800">${{ number_format($totalCostoHa, 2) }}/ha</span>
                    </div>
                @endif
            </div>
        @endif
